...
describe("Migration framework", () => {
  ...
  it("populates junction tables from existing JSON data", () => {
    ...
    const tueRows = db.prepare(
      "SELECT * FROM text_unit_entities WHERE text_unit_id = 'tu:001'"
    ).all();
    expect(tueRows).toHaveLength(2);

    const ceRows = db.prepare(
      "SELECT * FROM community_entities WHERE community_id = 'comm:0:0'"
    ).all();
