...
Call `hald_stats` to check if the index exists. If the tool returns an error containing "no such table", "unable to open", or "No index found", the repo hasn't been indexed yet — tell the user and suggest running `hald_index` (see the hald-index skill).
...
1. **Check index first.** If unsure whether the repo is indexed, call `hald_stats`. If the index doesn't exist or is empty, suggest `hald_index` before proceeding.
2. **Choose the right tool.** Match the question type:
   - Who knows X? → `hald_find_expert`
   - Why did we...? → `hald_trace_decision`
   - What changes with X? → `hald_show_coupling`
   - What's our bus factor? → `hald_find_silos`
   - How is X connected to Y? → `hald_get_path`
   - Tell me about entity X → `hald_get_entity`
   - Everything else → `hald_query`
...
**You:** Call `hald_find_expert` with module `src/billing`. Synthesize: who knows the code, how recently active, which specific areas they've touched.

**User:** "Why did we switch from Webpack to Vite?"
**You:** Call `hald_trace_decision` with topic "Webpack to Vite migration". Synthesize the timeline: who drove it, when, and what commits reveal about the motivation.

**User:** "Give me an overview of the codebase architecture"
**You:** Call `hald_query` with the question (global search over community summaries). Synthesize the top communities into an architecture overview.

**User:** "What's the bus factor for our payment system?"
**You:** Call `hald_find_silos`. Filter results for payment-related modules. Report which modules have single maintainers and who they are.

**User:** "How is Alice connected to the auth module?"
**You:** Call `hald_get_path` with from "Alice" and to "auth". Walk through the path: Alice authored module X, which co-changes with auth.

**User:** "Tell me everything about the database layer"
**You:** Call `hald_get_entity` with query "database layer". Report its type, description, relationships, and activity timeline.
