...
import { formatLocalResult, formatGlobalResult, formatDecisionTrace } from "./formatters.js";

type GetStore = () => Store;
type GetQueryEmbedder = () => Promise<QueryEmbedder>;

/** Max relations rendered by hald_get_entity before footering the rest — bounds hub-entity output. */
const MAX_ENTITY_RELATIONS = 50;
  ...
  server.registerTool(
    ...
    async ({ query }) => {
      try {
        ...
        // Show relations grouped by type. getRelationsForEntity has no LIMIT,
        // so a hub entity (e.g. a prolific author wired to hundreds of modules)
        // would otherwise dump unbounded text into the host context. Mirror the
        // bounding discipline in local search: keep the top relations by weight
        // and footer the remainder.
        const rels = store.getRelationsForEntity(entity.id);
        if (rels.length > 0) {
          lines.push("", `### Relationships (${rels.length})\n`);

          const sortedRels = [...rels].sort((a, b) => b.weight - a.weight);
          const shownRels = sortedRels.slice(0, MAX_ENTITY_RELATIONS);
          const hiddenCount = sortedRels.length - shownRels.length;

          const relsByType = new Map<string, typeof rels>();
          for (const rel of shownRels) {
          ...
          if (hiddenCount > 0) {
            lines.push(
              `…and ${hiddenCount} more relationship${hiddenCount === 1 ? "" : "s"} (showing top ${MAX_ENTITY_RELATIONS} by weight).`,
            );
          }
        }
